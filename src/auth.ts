    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
       ...
       const key = `login:${(credentials.username as string).toLowerCase()}`;
        ...
        return { id: user.id, name: user.name, username: user.username, role: user.role, avatarId: user.avatarId } as any;
      },
    }),
  ],
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      ...
      if (now - lastFetch > 5 * 60 * 1000) {
        try {
          ...
          if (fresh) { token.avatarId = fresh.avatarId; token.name = fresh.name; token.role = fresh.role; token.fetchedAt = now; }
        } catch {}
      }
      return token;
    },
