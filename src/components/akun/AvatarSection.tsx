...
function CooldownBadge({ sisaHari }: { sisaHari: number }) {
  return (
    <div className="flex items-center gap-2 rounded-xl px-3 py-2 text-xs border"
      style={{ backgroundColor: "var(--bg-secondary)", borderColor: "var(--border)", color: "var(--text-faint)" }}>
      <svg className="w-4 h-4 text-tcb-red flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      Upload avatar tersedia lagi dalam{" "}
      <span className="font-bold" style={{ color: "var(--text-primary)" }}>{sisaHari} hari</span>
    </div>
  );
}
...
export function AvatarSection({
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!["image/jpeg", "image/jpg", "image/png"].includes(f.type)) {
      setMsg({ text: "Format file harus JPG atau PNG", ok: false }); return;
    }
    if (f.size > 2 * 1024 * 1024) {
      setMsg({ text: "Ukuran file maksimal 2MB", ok: false }); return;
    }
  ...
  async function handleUpload() {
    ...
    if (res.ok) {
      ...
      setFile(null); setPreview(null); router.refresh();
    } else {
  ...
  async function handleSavePreset() {
    ...
    setSaving(false);
    if (res.ok) { setMsg({ text: "Avatar diperbarui!", ok: true }); router.refresh(); }
    else { setMsg({ text: "Gagal menyimpan. Coba lagi.", ok: false }); }
  }
  ...
  return (
    <div className="card">
      ...
      {/* Tab */}
      <div className="flex gap-1 mb-4 rounded-xl p-1" style={{ backgroundColor: "var(--bg-secondary)" }}>
        {(["upload", "preset"] as const).map((t) => (
          <button key={t} onClick={() => { setTab(t); setMsg(null); }}
            className="flex-1 py-2 rounded-lg text-sm font-semibold transition-all"
            style={tab === t
              ? { backgroundColor: "#E01E2B", color: "#fff" }
          ...
          {!canUpload ? (
            ...
            <>
              <div onClick={() => fileRef.current?.click()}
                ...
                onMouseEnter={(e) => (e.currentTarget.style.borderColor = "#E01E2B")}
                onMouseLeave={(e) => (e.currentTarget.style.borderColor = "var(--border-mid)")}>
                {preview ? (
                  <img src={preview} alt="Preview"
                    className="w-24 h-24 rounded-full object-cover mx-auto border-2 border-tcb-red" />
                ) : (
                  <>
                    <svg className="w-8 h-8 mx-auto mb-2 transition-colors" fill="none" viewBox="0 0 24 24"
                      stroke="currentColor" style={{ color: "var(--text-faint)" }}>
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    <p className="text-sm transition-colors" style={{ color: "var(--text-faint)" }}>
                      Klik untuk pilih foto
                    </p>
                    <p className="text-xs mt-1" style={{ color: "var(--border-mid)" }}>
                      JPG, PNG · Maks 2MB
                    </p>
                  </>
                )}
        ...
        <div>
          {avatars.length === 0 ? (
            <p className="text-sm" style={{ color: "var(--text-faint)" }}>
              Belum ada avatar preset. Hubungi admin.
            </p>
          ) : (
            <>
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mb-4">
                {avatars.map((av) => (
                  <button key={av.id} type="button" onClick={() => setSelected(av.filename)}
                    className="relative rounded-xl overflow-hidden border-2 transition-all aspect-square"
                    style={selected === av.filename
                      ? { borderColor: "#E01E2B", boxShadow: "0 0 10px rgba(224,30,43,0.35)" }
                      : { borderColor: "var(--border-mid)" }
                    }>
                    <img src={`/avatars/${av.filename}`} alt={av.filename}
      ...
      {msg && (
        <p className={`text-xs mt-3 ${msg.ok ? "text-green-500" : "text-red-400"}`}>{msg.text}</p>
      )}
    </div>
  );
}
